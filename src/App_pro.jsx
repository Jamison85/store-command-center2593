...
const TASK_FILTERS = ["Open", "Must Do", "Done", "All"];
...
export default function App() {
  ...
  const [person, setPerson] = useSavedState("caseys_person_v4", "Jamo");
  ...
  const stats = useMemo(() => {
    ...
    const tasksOpen = tasks.filter((task) => task.status !== "Done").length;
    const mustDoOpen = tasks.filter((task) => task.status !== "Done" && task.priority === "Must Do").length;
    ...
    <section className="screen">
      <header className="hero-card command-hero">
        <div>
          ...
          <p className="muted">
            {shiftType} · {staffing.label} · {formatDateTime()}
          </p>
        </div>
      ...
      <article className="card command-card">
        ...
        <div className="grid-3 top-space">
          <select className="input" value={person} onChange={(event) => setPerson(event.target.value)}>
            {PEOPLE.map((name) => (
...
function Tasks({ tasks, setTasks }) {
  ...
  const filtered = tasks.filter((task) => {
    ...
    if (filter === "Must Do") return task.priority === "Must Do" && task.status !== "Done";
  ...
  return (
    <section className="screen">
      <PageHeader label="Task Board" title="Tasks" subtitle="A place for loose ends, because brains are unreliable meat notebooks." />
      ...
      {formOpen && (
        <form className="card form-card" onSubmit={saveTask}>
          ...
          <textarea className="textarea" placeholder="Description" value={form.description} onChange={(event) => setForm({ ...form, description: event.target.value })} />
      ...
      <div className="stack">
        ...
        {filtered.map((task) => (
          <article key={task.id} className={`card task-card ${task.priority === "Must Do" && task.status !== "Done" ? "must-card" : ""}`}>
            <div className="card-header">
              <div>
                <h2 className={task.status === "Done" ? "done-text" : ""}>{task.title}</h2>
                <p className="muted">{task.description || "No details added."}</p>
              </div>
              <span className={`badge ${task.priority === "Must Do" ? "danger" : task.priority === "Should Do" ? "warning" : "success"}`}>
                {task.priority}
              </span>
            </div>
            ...
            <select className="input" value={task.status} onChange={(event) => updateTask(task.id, "status", event.target.value)}>
              <option>Not Started</option>
              <option>In Progress</option>
              <option>Done</option>
            </select>
    ...
    <section className="screen">
      ...
      <article className="card command-card">
        ...
        <div className="handoff-stats">
          ...
          <span>Done <b>{completed.length}</b></span>
...
function buildActionQueue({ stats, dateWatch, tasks, walkState, staffingMode, shiftType }) {
  ...
  tasks
    .filter((task) => task.priority === "Must Do" && task.status !== "Done")
    .slice(0, 4)
