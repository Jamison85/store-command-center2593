...
const ICONS = {
  ...
  quick: (
    <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M13 2 4 14h7l-1 8 10-13h-7z"/></svg>
  ),
  inbox: (
    <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 4h16l-2 11H6z"/><path d="M6 15h3a3 3 0 0 0 6 0h3"/><path d="M4 20h16"/></svg>
  ),
  issue: (
    <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M14.7 6.3a4 4 0 0 0-5 5L4 17v3h3l5.7-5.7a4 4 0 0 0 5-5l-2.6 2.6-2.9-2.9z"/></svg>
  ),
  audit: (
    <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M9 4h6l1 2h3v15H5V6h3z"/><path d="M9 12l2 2 4-4"/><path d="M8 18h8"/></svg>
  ),
  settings: (
    <svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.7 1.7 0 0 0 .3 1.9l.1.1-2.1 2.1-.1-.1a1.7 1.7 0 0 0-1.9-.3 1.7 1.7 0 0 0-1 1.5V21h-3v-.8a1.7 1.7 0 0 0-1-1.5 1.7 1.7 0 0 0-1.9.3l-.1.1-2.1-2.1.1-.1A1.7 1.7 0 0 0 5 15a1.7 1.7 0 0 0-1.5-1H3v-3h.5A1.7 1.7 0 0 0 5 10a1.7 1.7 0 0 0-.3-1.9l-.1-.1 2.1-2.1.1.1A1.7 1.7 0 0 0 8.7 6a1.7 1.7 0 0 0 1-1.5V4h3v.5a1.7 1.7 0 0 0 1 1.5 1.7 1.7 0 0 0 1.9-.3l.1-.1 2.1 2.1-.1.1A1.7 1.7 0 0 0 19 10a1.7 1.7 0 0 0 1.5 1h.5v3h-.5A1.7 1.7 0 0 0 19.4 15z"/></svg>
  ),
};
...
const QUICK_TASKS = [
  { title: "Restroom check", description: "Cleanliness, trash, supplies, floors.", priority: "Must Do", timing: "Today", assignee: "Team Member" },
  { title: "Coffee area reset", description: "Coffee, cups, lids, creamers, counters, and machines.", priority: "Must Do", timing: "Before 10 AM", assignee: "Me" },
  { title: "Cooler recovery", description: "Fill and face high-traffic cooler doors.", priority: "Should Do", timing: "Today", assignee: "Center Store" },
  { title: "Water check", description: "Fill water displays and note bulk backstock.", priority: "Should Do", timing: "Today", assignee: "Center Store" },
  { title: "Trash run", description: "Interior, exterior, fuel court, and bathrooms.", priority: "Should Do", timing: "Today", assignee: "Team Member" },
  { title: "Lottery check", description: "Verify tickets, counts, and any mismatch follow-up.", priority: "Must Do", timing: "Before Leaving", assignee: "Me" },
  { title: "Cigarette audit", description: "Check counts, outs, and backbar standards.", priority: "Must Do", timing: "Today", assignee: "Me" },
  { title: "Kitchen follow-up", description: "Production, food safety, waste, or kitchen note.", priority: "Should Do", timing: "Today", assignee: "Kitchen" },
  { title: "Vendor issue", description: "Document vendor problem and add to Issue Log if needed.", priority: "Should Do", timing: "Today", assignee: "Me" },
];

const QUICK_DATE_ITEMS = [
  { name: "Creamer", area: "Dairy", issueType: "None" },
  { name: "Open-air cooler item", area: "Open-Air Cooler", issueType: "None" },
  { name: "Bakery item", area: "Bakery", issueType: "None" },
  { name: "Kitchen prep item", area: "Kitchen", issueType: "None" },
  { name: "Vendor item", area: "Vendor", issueType: "Vendor Issue" },
];

const DEFAULT_ISSUES = [];

const DEFAULT_AUDIT_ITEMS = [
  { id: createId(), area: "Lottery", title: "Lottery audit / variance check", status: "Open", notes: "", addToHandoff: true },
  { id: createId(), area: "Cigarettes", title: "Cigarette count / backbar check", status: "Open", notes: "", addToHandoff: true },
  { id: createId(), area: "Cash / Safe", title: "Safe, drawers, and change order check", status: "Open", notes: "", addToHandoff: false },
  { id: createId(), area: "Receiving", title: "Receiving / invoice follow-up", status: "Open", notes: "", addToHandoff: false },
  { id: createId(), area: "MAP / Displays", title: "MAP, power wings, and displays check", status: "Open", notes: "", addToHandoff: false },
  { id: createId(), area: "Vendor", title: "Vendor follow-up review", status: "Open", notes: "", addToHandoff: false },
];
...
export default function App() {
  ...
  const [issueLog, setIssueLog] = useSavedState("caseys_issue_log_finished_v1", DEFAULT_ISSUES);
  const [auditItems, setAuditItems] = useSavedState("caseys_audit_items_finished_v1", DEFAULT_AUDIT_ITEMS);
  const [displayMode, setDisplayMode] = useSavedState("caseys_display_mode_finished_v1", "comfortable");
  ...
  const stats = buildStats(session, dateWatchWithStatus, issueLog, auditItems);
  ...
  return (
    <div className={`app-shell ${displayMode === "compact" ? "compact-mode" : ""}`}>
      <main className="app-main">
        ...
        {screen === "carryover" && <CarryoverInbox {...context} />}
        {screen === "quick-add" && <QuickAddHub {...context} />}
        {screen === "issues" && <IssueLog {...context} />}
        {screen === "audit" && <AuditBoard {...context} />}
        {screen === "settings" && <DisplaySettings {...context} />}
...
function Handoff({ manager, session, stats, dateWatch, issueLog, auditItems, handoffHistory, setHandoffHistory }) {
  const handoffData = makeHandoffData({ manager, session, stats, dateWatch, issueLog, auditItems });
  ...
  return (
    <section className="screen">
      ...
      <HandoffSection title="Store Issues" items={handoffData.activeIssues.map((issue) => `${issue.area}: ${issue.title}${issue.status ? ` (${issue.status})` : ""}${issue.ticket ? `, ticket ${issue.ticket}` : ""}${issue.notes ? `, note: ${issue.notes}` : ""}`)} />
      <HandoffSection title="Audit Follow-ups" items={handoffData.auditFollowUps.map((item) => `${item.area}: ${item.title}${item.status ? ` (${item.status})` : ""}${item.notes ? `, note: ${item.notes}` : ""}`)} />
...
function EndShiftReview({ manager, session, setSession, stats, dateWatch, issueLog, auditItems, setScreen, setDailyLog, setHandoffHistory, handoffHistory }) {
...
function CarryoverInbox({ session, setSession, setScreen }) {
  const carryovers = (session.tasks || []).filter((task) => task.source === "Carryover" || task.carriedFrom);

  function updateTask(taskId, updates) {
    setSession({
      ...session,
      tasks: session.tasks.map((task) => (task.id === taskId ? { ...task, ...updates } : task)),
    });
  }

  function removeTask(taskId) {
    setSession({ ...session, tasks: session.tasks.filter((task) => task.id !== taskId) });
  }

  return (
    <section className="screen">
      <PageHeader icon={ICONS.inbox} label="Carryover Inbox" title="Yesterday’s unfinished work" subtitle="Review carryovers before they quietly become tomorrow’s problem wearing a fake mustache." />

      <article className="card command-card">
        <div className="card-header">
          <div>
            <p className="eyebrow">Carryover Summary</p>
            <h2>{carryovers.length} items waiting</h2>
            <p className="muted">Accept, complete, handoff, or drop anything that no longer matters.</p>
          </div>
          <button className="primary-btn" onClick={() => setScreen("tasks")}>Open Tasks</button>
        </div>
      </article>

      <div className="stack">
        {carryovers.length === 0 && <EmptyState title="No carryovers" text="Clean slate. Suspicious, but useful." />}
        {carryovers.map((task) => (
          <article key={task.id} className="card carryover-card">
            <div className="card-header">
              <div>
                <p className="eyebrow">From {task.carriedFrom || task.sessionDate || "Previous Day"}</p>
                <h2>{task.title}</h2>
                <p className="muted">{task.description || "No details added."}</p>
              </div>
              <span className={`badge ${statusBadgeClass(task.status)}`}>{task.status}</span>
            ...
            <div className="action-row">
              <button className="primary-btn" onClick={() => updateTask(task.id, { source: "Accepted", status: "To Do" })}>Accept for Today</button>
              <button className="ghost-btn" onClick={() => updateTask(task.id, { status: "Complete" })}>Complete</button>
              <button className="ghost-btn" onClick={() => updateTask(task.id, { addToHandoff: true })}>Add to Handoff</button>
              <button className="danger-btn" onClick={() => removeTask(task.id)}>Drop</button>
            </div>
          </article>
        ))}
      </div>
...
function QuickAddHub({ session, setSession, rawDateWatch, setDateWatch, issueLog, setIssueLog, setScreen, manager }) {
  function addTask(template) {
    ...
    setSession({ ...session, tasks: [task, ...session.tasks] });
  }

  function addDateItem(template) {
    setDateWatch([
      {
        id: createId(),
        ...template,
        ...
        notes: "Quick added. Confirm details.",
        resolved: false,
      },
      ...rawDateWatch,
    ]);
  ...
  return (
    <section className="screen">
      <PageHeader icon={ICONS.quick} label="Quick Add" title="Fast capture" subtitle="Add common work without typing a tiny novel on a phone. A rare act of mercy." />

      <article className="card command-card">
        <div className="card-header">
          <div>
            <p className="eyebrow">Quick Task</p>
            <h2>Add a common task</h2>
          </div>
          <button className="ghost-btn" onClick={() => setScreen("tasks")}>Task Board</button>
        </div>
        <div className="quick-add-grid">
          {QUICK_TASKS.map((task) => (
            <button key={task.title} onClick={() => addTask(task)}>
              <span className="icon-chip">{ICONS.tasks}</span>
              <strong>{task.title}</strong>
      ...
      <article className="card">
        <div className="card-header">
          <div>
            <p className="eyebrow">Quick Date Watch</p>
            <h2>Add an item to watch</h2>
          </div>
          <button className="ghost-btn" onClick={() => setScreen("dates")}>Date Watch</button>
        </div>
        <div className="quick-add-grid">
          {QUICK_DATE_ITEMS.map((item) => (
            <button key={`${item.name}-${item.area}`} onClick={() => addDateItem(item)}>
              <span className="icon-chip">{ICONS.dates}</span>
              <strong>{item.name}</strong>
      ...
      <article className="card">
        <div className="card-header">
          <div>
            <p className="eyebrow">Quick Issue</p>
            <h2>Add issue log item</h2>
          </div>
          <button className="ghost-btn" onClick={() => setScreen("issues")}>Issue Log</button>
        </div>
        <div className="quick-add-grid">
          {["Coffee", "Cooler", "Register", "Fuel", "Vendor", "Kitchen", "Backroom", "Restroom"].map((area) => (
            <button key={area} onClick={() => addIssue(area)}>
              <span className="icon-chip">{ICONS.issue}</span>
              <strong>{area}</strong>
              <small>Issue</small>
            </button>
          ))}
...
function IssueLog({ issueLog, setIssueLog, manager }) {
  const blank = {
    title: "",
    area: "Center Store",
    type: "General",
  ...
  const filtered = issueLog.filter((issue) => {
    if (filter === "All") return true;
    if (filter === "Resolved") return issue.status === "Resolved";
    if (filter === "Urgent") return issue.status === "Urgent";
    return issue.status !== "Resolved";
  });

  function openAdd() {
    setEditingId(null);
    setForm({ ...blank, reportedBy: manager.name });
    setFormOpen(true);
  }
  ...
  function saveIssue(event) {
    ...
    if (editingId) {
      setIssueLog(issueLog.map((issue) => (issue.id === editingId ? { ...issue, ...form } : issue)));
    } else {
      setIssueLog([{ id: createId(), createdAt: new Date().toISOString(), ...form }, ...issueLog]);
    }
  ...
  function updateIssue(issueId, field, value) {
    setIssueLog(issueLog.map((issue) => (issue.id === issueId ? { ...issue, [field]: value } : issue)));
  }

  function deleteIssue(issueId) {
    setIssueLog(issueLog.filter((issue) => issue.id !== issueId));
  }

  return (
    <section className="screen">
      <PageHeader icon={ICONS.issue} label="Store Issue Log" title="Long-term problems" subtitle="For equipment, vendors, tickets, and follow-ups that should not disappear at midnight." />

      <article className="card command-card">
        <div className="card-header">
          <div>
            <p className="eyebrow">Issue Control</p>
            <h2>{issueLog.filter((issue) => issue.status !== "Resolved").length} open issues</h2>
          </div>
          <button className="primary-btn" onClick={openAdd}>Add Issue</button>
        </div>
      </article>

      <div className="filter-row">
        {["Open", "Urgent", "Resolved", "All"].map((chip) => (
          <button key={chip} className={`filter-chip ${filter === chip ? "active" : ""}`} onClick={() => setFilter(chip)}>{chip}</button>
        ))}
      </div>

      {formOpen && (
        <form className="card form-card" onSubmit={saveIssue}>
          <p className="eyebrow">{editingId ? "Edit Issue" : "New Issue"}</p>
          <input className="input" placeholder="Issue title" value={form.title} onChange={(event) => setForm({ ...form, title: event.target.value })} />
          <div className="grid-2">
            <input className="input" placeholder="Area" value={form.area} onChange={(event) => setForm({ ...form, area: event.target.value })} />
            <select className="input" value={form.type} onChange={(event) => setForm({ ...form, type: event.target.value })}>
              <option>General</option>
          ...
          <div className="grid-2">
            ...
            <input className="input" placeholder="Ticket number" value={form.ticket} onChange={(event) => setForm({ ...form, ticket: event.target.value })} />
          </div>
          <label className="field-label">Follow-up date</label>
          <input className="input" type="date" value={form.followUpDate} onChange={(event) => setForm({ ...form, followUpDate: event.target.value })} />
          <textarea className="textarea" placeholder="Notes" value={form.notes} onChange={(event) => setForm({ ...form, notes: event.target.value })} />
          <label className="handoff-toggle">
            <input type="checkbox" checked={!!form.addToHandoff} onChange={(event) => setForm({ ...form, addToHandoff: event.target.checked })} />
            Add to handoff
          ...
          <div className="action-row">
            <button className="primary-btn" type="submit">Save Issue</button>
            <button className="ghost-btn" type="button" onClick={() => setFormOpen(false)}>Cancel</button>
          </div>
        </form>
      )}
      ...
      <div className="stack">
        {filtered.length === 0 && <EmptyState title="No issues here" text="A beautiful lie, but a useful screen." />}
        {filtered.map((issue) => (
          <article key={issue.id} className={`card issue-card ${issue.status === "Urgent" ? "urgent-card" : ""}`}>
            <div className="card-header">
              <div>
                <p className="eyebrow">{issue.area} · {issue.type}</p>
                <h2>{issue.title}</h2>
                <p className="muted">{issue.notes || "No notes added."}</p>
              </div>
              <span className={`badge ${issue.status === "Resolved" ? "success" : issue.status === "Urgent" ? "danger" : "warning"}`}>{issue.status}</span>
            </div>
            <div className="task-meta">
              <span>Reported by {issue.reportedBy || "Unknown"}</span>
              <span>Follow-up {issue.followUpDate || "Not set"}</span>
              {issue.ticket && <span>Ticket {issue.ticket}</span>}
            </div>
            <div className="action-row">
              <button className="ghost-btn" onClick={() => openEdit(issue)}>Edit</button>
              <button className="ghost-btn" onClick={() => updateIssue(issue.id, "status", issue.status === "Resolved" ? "Open" : "Resolved")}>{issue.status === "Resolved" ? "Reopen" : "Resolve"}</button>
              <button className="danger-btn" onClick={() => deleteIssue(issue.id)}>Delete</button>
            </div>
          </article>
        ))}
      </div>
...
function AuditBoard({ auditItems, setAuditItems, setScreen }) {
  function updateAudit(itemId, field, value) {
    setAuditItems(auditItems.map((item) => (item.id === itemId ? { ...item, [field]: value } : item)));
  }

  function resetAuditBoard() {
    setAuditItems(DEFAULT_AUDIT_ITEMS.map((item) => ({ ...item, id: createId(), status: "Open", notes: "" })));
  }

  return (
    <section className="screen">
      <PageHeader icon={ICONS.audit} label="Audit Board" title="Manager checks" subtitle="Lottery, cigarettes, cash, receiving, MAP, and vendor follow-ups in one place." />

      <article className="card command-card">
        <div className="card-header">
          <div>
            <p className="eyebrow">Audit Summary</p>
            <h2>{auditItems.filter((item) => item.status === "Needs Follow-up").length} need follow-up</h2>
            <p className="muted">{auditItems.filter((item) => item.status === "Complete").length} complete · {auditItems.length} total</p>
          </div>
          <button className="ghost-btn" onClick={resetAuditBoard}>Reset Board</button>
        </div>
      </article>

      <div className="stack">
        {auditItems.map((item) => (
          <article key={item.id} className={`card audit-card ${item.status === "Needs Follow-up" ? "must-card" : ""}`}>
            <div className="card-header">
              <div>
                <p className="eyebrow">{item.area}</p>
                <h2>{item.title}</h2>
              </div>
              <span className={`badge ${item.status === "Complete" ? "success" : item.status === "Needs Follow-up" ? "danger" : "neutral"}`}>{item.status}</span>
            </div>
            <div className="grid-2 top-space">
              <select className="input" value={item.status} onChange={(event) => updateAudit(item.id, "status", event.target.value)}>
                <option>Open</option>
                <option>Good</option>
              ...
              <label className="handoff-toggle inline-toggle">
                <input type="checkbox" checked={!!item.addToHandoff} onChange={(event) => updateAudit(item.id, "addToHandoff", event.target.checked)} />
                Handoff
              </label>
            </div>
            <label className="field-label">Notes</label>
            <textarea className="textarea" value={item.notes} placeholder="Mismatch, follow-up, vendor issue, or details..." onChange={(event) => updateAudit(item.id, "notes", event.target.value)} />
            {item.status === "Needs Follow-up" && (
              <button className="ghost-btn full top-space" onClick={() => setScreen("handoff")}>Review in Handoff</button>
            )}
          </article>
        ))}
      </div>
...
function DisplaySettings({ displayMode, setDisplayMode }) {
  return (
    <section className="screen">
      <PageHeader icon={ICONS.settings} label="Display Settings" title="Make it usable on shift" subtitle="Comfortable for planning, compact for moving fast when the store starts acting like a circus with inventory." />

      <article className="card command-card">
        <p className="eyebrow">Display Mode</p>
        <h2>{displayMode === "compact" ? "Compact" : "Comfortable"}</h2>
        <p className="muted">Compact mode tightens spacing and reduces card size so more fits on your phone.</p>
        <div className="status-helper-row">
          <button className={`status-pill ${displayMode === "comfortable" ? "active" : ""}`} onClick={() => setDisplayMode("comfortable")}>
            <strong>Comfortable</strong>
            <span>More breathing room.</span>
          </button>
          <button className={`status-pill ${displayMode === "compact" ? "active" : ""}`} onClick={() => setDisplayMode("compact")}>
      ...
      <article className="card">
        ...
        <h2>Premium operations theme</h2>
        <ul className="simple-list">
          <li>Espresso background for contrast.</li>
          <li>Cream cards for readability.</li>
          <li>Orange for primary action.</li>
          <li>Blue for in progress.</li>
          <li>Sage for complete.</li>
          <li>Red only for urgency.</li>
        </ul>
      </article>
    </section>
  );
...
function More({ setScreen }) {
  const tools = [
    { title: "Quick Add", text: "Add common tasks, date items, and issues fast.", screen: "quick-add", icon: ICONS.quick },
    { title: "Carryover Inbox", text: "Review unfinished work from previous sessions.", screen: "carryover", icon: ICONS.inbox },
    { title: "Issue Log", text: "Track equipment, vendor, ticket, and follow-up problems.", screen: "issues", icon: ICONS.issue },
    { title: "Audit Board", text: "Lottery, cigarettes, receiving, cash, MAP, and vendor checks.", screen: "audit", icon: ICONS.audit },
    ...
    { title: "Display Settings", text: "Switch between comfortable and compact layouts.", screen: "settings", icon: ICONS.settings },
  ];

  return (
    <section className="screen">
      <PageHeader icon={ICONS.more} label="More Tools" title="Operations Layer" subtitle="The extra tools that turn this from a nice checklist into an actual store system." />
...
function buildStats(session, dateWatch, issueLog = [], auditItems = []) {
  ...
  const openIssues = issueLog.filter((issue) => issue.status !== "Resolved").length;
  const urgentIssues = issueLog.filter((issue) => issue.status === "Urgent").length;
  const auditFollowUps = auditItems.filter((item) => item.status === "Needs Follow-up").length;
  const auditOpen = auditItems.filter((item) => item.status !== "Complete").length;
...
function buildActionQueue({ session, manager, stats, dateWatch, issueLog = [], auditItems = [] }) {
  const queue = [];
  ...
  issueLog
    .filter((issue) => issue.status === "Urgent")
    .slice(0, 3)
    .forEach((issue) => queue.push({
      id: `issue-${issue.id}`,
      priority: "Issue",
      title: `Issue needs follow-up: ${issue.title}`,
      detail: `${issue.area}${issue.ticket ? ` · Ticket ${issue.ticket}` : ""}${issue.notes ? ` · ${issue.notes}` : ""}`,
      screen: "issues",
      tone: "danger",
    }));

  auditItems
    .filter((item) => item.status === "Needs Follow-up")
    .slice(0, 3)
    .forEach((item) => queue.push({
      id: `audit-${item.id}`,
      priority: "Audit",
      title: `Audit follow-up: ${item.area}`,
      detail: item.notes || item.title,
      screen: "audit",
      tone: "warning",
    }));
  ...
  const priorityOrder = { Critical: 0, Urgent: 1, Issue: 2, Today: 3, "Before 10": 4, "Must Do": 5, Audit: 6, Truck: 7, Survival: 8, Staffing: 9, Role: 10, Soon: 11, Steady: 12 };
  return queue.sort((a, b) => (priorityOrder[a.priority] ?? 99) - (priorityOrder[b.priority] ?? 99));
}

function makeHandoffData({ manager, session, stats, dateWatch, issueLog = [], auditItems = [] }) {
  const staffing = STAFFING_MODES.find((mode) => mode.id === session.staffingMode) || STAFFING_MODES[0];
  ...
  const activeIssues = issueLog.filter((issue) => issue.status !== "Resolved");
  const auditFollowUps = auditItems.filter((item) => item.status === "Needs Follow-up" || item.addToHandoff);

  return { manager, session, stats, staffing, completed, open, dateRisks, walkNotes, teamForHandoff, truckForHandoff, activeIssues, auditFollowUps };
}

function buildHandoffText({ manager, session, stats, staffing, completed, open, dateRisks, walkNotes, teamForHandoff, truckForHandoff, activeIssues = [], auditFollowUps = [] }) {
  const lines = [];
  ...
  lines.push("Store Issues:", ...listOrNone(activeIssues.map((issue) => `- ${issue.area}: ${issue.title}${issue.status ? ` (${issue.status})` : ""}${issue.ticket ? `, ticket ${issue.ticket}` : ""}${issue.notes ? `, note: ${issue.notes}` : ""}`)), "");
  lines.push("Audit Follow-ups:", ...listOrNone(auditFollowUps.map((item) => `- ${item.area}: ${item.title}${item.status ? ` (${item.status})` : ""}${item.notes ? `, note: ${item.notes}` : ""}`)), "");
